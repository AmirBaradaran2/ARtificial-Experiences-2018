...
//	}
	...
	private IEnumerator DownloadMediaForUsername (string username)
	{
		...
		do {
			...
			data = JsonUtility.FromJson<topLevelData_mosaic> (json);
			...
			// To pull higher-res images_mosaic or actual video files, change the arguments below.
			foreach (items_mosaic item in data.items_mosaic) {
				count += 1;
				string imgURL = item.images_mosaic.low_resolution.url;
			...
			//yield 
			string lastItemID = data.items_mosaic [data.items_mosaic.Count - 1].id;
			queryOptions = "?max_id=" + lastItemID;

			if (count > 200) {
				break;
			}
			// if there are more images_mosaic, pull the next page
...
public struct topLevelData_mosaic
{
	public string status;
	public bool more_available;
	public List<items_mosaic> items_mosaic;
}

[System.Serializable]
public class items_mosaic
{
	public string id;
	public images_mosaic images_mosaic;

}

[System.Serializable]
public class images_mosaic
{
	public lowResolution_mosaic low_resolution;

}

[System.Serializable]
public class lowResolution_mosaic
{
	public int height;
	public int width;
	public string url;
